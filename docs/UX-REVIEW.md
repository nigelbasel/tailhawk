...
- **The keyboard is the weakest surface.** Beyond the two dead keys above: the filter panel is drawn
  ...
  variant (*fixed 2026-09-15 — the panel is a native list, reached by F6*); column width and order
  are drag-only; `Ctrl+\` uses a character the guide names as
  locale-specific.
- **Accessibility is a stated gap, not an unknown one.** The provider deliberately excludes the grid
  ...
  *large* findings in the whole list, and this is one of them. *Fixed 2026-09-15: the grid is a
  Document with the Text pattern, and both large findings are now done. What is knowingly left is
  the live tail's own announcement — §14.1's "read new lines" — which waits for a later part.*
...
| 3 | Column header context menu — **Fixed 2026-09-17**, and **not** the way this row prescribed | Ensuring keyboard access (for example, a tab stop for every interactive control) so that users can accomplish the same things in your program with eit… | menubar::header_context (menubar.rs:187-219) carries `Sort &ascending`, `Sort &descending`, `Top &N…`, `&Filter on {title}…` and `Clear s&ort`. It has exactly one construction site — context_menu's Un… | ~~Make the Shift+F10 anchor follow the focused surface~~ — **unimplementable as written, and the sizing was wrong.** A `SysHeader32` handles no keyboard messages and draws no focus indicator (its documented default processing lists none), so `WS_TABSTOP` would put focus on a control that shows nothing and answers no arrow key — satisfying the letter of this rule while breaking the same page's "always show the input focus indicator". And there is no focused surface to follow: selection here is *rows*, and nothing in the model names a current column. The four acts carry a payload, so `Command::LISTED` cannot hold them and they can have no register id. Done instead as three Format submenus listing the shown columns — `Sort by`, `Top 50 by`, `Filter on` — id'd by position and resolved through `Document::column_at`. | ~~small~~ medium |
| 4 | Filter panel — **Fixed 2026-09-15**: a check-box list view in a child window, reached by F6; the drawn buttons became Edit ▸ Edit filter… and Remove filter | Assign tabs stops to all interactive controls, including read-only edit boxes. | The whole panel is painted glyphs with click rectangles. `Clear all`, `Remove`, `Edit…` and `Add…` are drawn by controls::button into panel_hits (main.rs:551-563); each chip row's `[x]` mark, `+`/`−`… | Rebuild the panel's title row as real Button controls and the chip list as a SysListView32 with check boxes inside a child window with WS_TABSTOP, so tab order, access keys and accessibility come from… | large |
| 5 | Log grid (WM_GETOBJECT provider) — **Fixed 2026-09-15**: each pane is a *Document* with the Text pattern over `textunit` and `gridtext`; rows off the screen are read on demand, the caret is a degenerate range and moves the system caret, and `TextSelectionChanged` follows it | Enabling programmatic access to all UI elements and text (for example, using the Active Accessibility COM interface, IAccessible). | The provider deliberately excludes the grid. main.rs:6845 — "The grid's text is `[v2]`'s virtualised text provider and is not here." uia::children (main.rs:6901-6917) returns only Kind::Tab(i), Kind::… | Implement ITextProvider/ITextRangeProvider over the visible row window (or IGridProvider + IValueProvider per cell once columns exist), backed by the same RowSource::row_text the painter reads, so wha… | large |
...
| 28 | Export / tee status bar text — **Fixed 2026-09-17, and this row undercounted it.** Three fragments were named; there were **six** sites saying "1 lines", including the file summary that is on screen for every document all the time, and the two counts `Finder::describe` appends — neither of which any test touched. Counted exactly: **six format strings carrying seven counts** — the tee's three, the file summary's *two* (lines and bytes), and `Finder::describe`'s scanning and too-slow counts — and one pure `counted(n, noun)` now serves all six (`1 line`, `2 lines`, `0 lines` because English counts zero with the plural, and `matches` rather than `matchs`). **`Finder`'s two count lines were excluded at first on the grounds that they already had the singular right, and a review proved they did not.** `status_line`'s explicit one-match arms applied only to a *finished* pass, so one match found mid-pass read **"1 matches so far"**, and `describe`'s no-current arm read **"— 1 matches"** — the case a no-wrap search reaches when its only match sits before the start row. No test that existed could reach either. They take the noun rule through a `noun_for(n, noun)` that gives the noun without the number, which is what they need, because `count_text` has already put §6's hedge where the digits go; and `status_line`'s two special-case arms are gone, since two copies of one rule is how the defect survived. The old assertion said `✓ exported 1 lines` and passed for as long as the defect existed | Accurate. Users should feel reassured that the information is technically accurate. | Confirmed at main.rs:1559-1562: format!("⇥ saving → {} ({} lines) — ", t.name(), t.written), format!("✓ exported {} lines → {} — ", t.written, t.name()) and format!("⇥ exporting → {} ({} lines) — ", …… | Give the three tee fragments the singular arm Finder::status_line already has, and update the assertion at main.rs:12162. | small |
...
- **Part two's first and third defects are fixed**: `Ctrl+H` became `Ctrl+K` and is bound, `F1`
  opens the keyboard map, and the header's context menu can be opened at all. **Its keyboard half
  is done too, as of 2026-09-17 — by another route.** `Shift+F10` still resolves to the grid, and
  deliberately so: the header cannot usefully take focus, because a `SysHeader32` implements no
  keyboard messages and draws no focus indicator, and there is no "current column" in a model whose
  selection is rows. The five acts reach the keyboard through **Format** instead — `Sort by ▸`,
  `Top 50 by ▸` and `Filter on ▸` list the shown columns, alongside the `Select columns…`,
  `Reset columns` and `Clear sort` that were already there. `Sort by` delegates to the same
  `Document::cycle_sort` a header click uses, so the menu and the header cannot drift apart.
