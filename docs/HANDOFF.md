...
| **What the picker does not do yet** | The label call and the opening pull both run **on the UI thread**, so a slow server holds the window between the menu click and the dialog — two round trips before it appears, and two more behind it. `open_remote`'s own doc-comment says a fetch does not belong there; it was already true of the pull and the picker adds to it. Separate windows is capped at `apps::MAX_SEPARATE` (8) because each is a tail with its own poll and its own bounded spill; the refusal says so and names Interleave as the remedy. |
