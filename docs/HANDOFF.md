...
| The column header's context menu could not be opened at all | Fixed: `header::item_at` answers `HDM_HITTEST` and `WM_CONTEXTMENU` routes to the document that owns the control. **Its keyboard half is done too, 2026-09-17, but not by making the header a tab stop.** That remedy is unimplementable: a `SysHeader32` handles no keyboard messages and draws no focus indicator, so focusing it would show the user nothing and answer no arrow key; and the four column-scoped acts need a subject column, which a row-selection model does not have. They are three **Format** submenus now — `Sort by ▸`, `Top 50 by ▸`, `Filter on ▸` — listing the shown columns, id'd by position in `header_columns()` and resolved through the pure `Document::column_at`. `Sort by` calls `Document::cycle_sort`, the same three-state cycle a header click runs, so the two cannot diverge. |
...
| | |
|---|---|
| **Loki's silent truncation is said — 2026-09-16** | `LOKI.md` §6's most important rule, built: `tail::was_cut` is asked by the opening window and by every poll, the worker publishes it beside the lag, and `Document::answers_cut` carries it and never clears — the window it happened in stays in the scrollback. The status bar says it standing, with the remedy, and the filter's `N of M`, the title's `N matches` and the Find dialog's own count all read **at least N** while it stands. **Declined deliberately, from the review:** the document's own `N lines` and the `of M` denominator stay exact, because they are exact facts about what Tailhawk holds — hedging them would be less honest, not more, and §6 is about counts that answer "how many matching records exist". **The real gap hedging cannot close:** `no matches` over a cut window is the most misleading answer of all and still reads plainly; saying "none in what was fetched" is the fix, and it is not built. |
| **A record that reaches Loki late is missed — mostly closed since, and the row was stale** | Both remedies are built and have been for some time: `tail::window_after` ends each window `LAG_NANOS` (5 s) short of the clock and starts it `OVERLAP_NANOS` (1 s) before the newest record held, with `Recent` keeping the repeats off the spill and a floor stopping a busy source pinning the tail. **What is left** is the gap between that and the estate's measured worst case: a record indexed more than ~6 s after its own timestamp is still never asked for again. The obvious enlargement does not work — five seconds of overlap at ~900 records/s was 4,500 repeats against a 2,000 limit, which stalled the tail — so the remedy is a slower sweep re-asking an older band on its own cadence, de-duplicated by the ring that already exists. **Closed 2026-09-16 by raising the lag, and a sweep was built and abandoned on the way.** The sweep — a slower second look at the band behind the tail — was written, tested and reviewed, and the review killed it with arithmetic: a sweep affords 2,000 records a minute, about 33 a second, against a source producing 900 a second, so re-reading a ninety-second band at full fidelity means ~81,000 records per sweep, more than the live tail fetches. With `Direction::Forward` and no cursor it re-asked the same leading ~2.2 s every cycle — roughly 4% of the band — and its answer, coming back at the limit every time, latched the "answers cut" banner permanently on for a reason the user could not see. **So the lag does the work instead:** `LAG_NANOS` is the estate's measured worst-case index delay rather than five seconds, which is complete instead of partial and needs no de-duplication, and the cost is stated rather than hidden — records appear that far after they happen, and the status bar says so. The abandoned sweep is in the session scratchpad if the shape is ever wanted for a quiet source. |
...
| **`current to 2s ago` is shown — 2026-09-07, reworded 2026-09-17** | `UI-DESIGN.md` §4's status-bar lag, for a remote source: the worker publishes the timestamp of the newest record it has written and the follow tick reads it onto the document, matched by spill directory. It is the **age of the newest record**, not the time since the last poll, so a quiet source reads as quiet and a tail that is falling behind says so and keeps saying it. `tail::lag_text` is the formatting and is pinned by test — sub-second is `<1s` because a five-second poll behind a thirty-second window has no better precision, and that case is now out of a running tail's reach entirely, kept only for a server clock that disagrees with ours. **The words were `lagging 2s` until 2026-09-17.** True, but a verdict: the window is held thirty seconds back of the clock by design, so the old wording accused a perfectly healthy tail of a fault once every follow tick. `current to 2s ago` states the same number as a fact and answers what someone watching a deploy is actually asking — how far has this read up to. Not `complete to`, which would promise nothing ever arrives behind the mark. **A paused tail shows no figure at all**, decided at the same time: `doc.lag` tracks the tail's head and not the rows on screen, so beside `Ctrl+End to follow` it described the state the reader had just been told how to get back to. The *answers cut* warning does stay on a paused line, because that one is about the scrollback they are actually reading. |

| **"Follow this trace" is built — 2026-09-09** | `Ctrl+T`, the row menu and Edit: the caret line's `trace_id` becomes an include chip, so the view is every line of that call across every service in the window. `trace.rs` reads it and refuses a value that is not plausibly an id; a line without one says so. Rung one of `UI-DESIGN.md` §7, and the only rung wanted. |
...
| **Closing a tab stops its tail — 2026-09-07** | It used to stop nothing: the worker kept asking Loki every five seconds for a document nobody could see, and its spill kept growing to its own bound, held until the window closed. `Shell::retire_closed_tails` drops the tail and then the spill of every remote source no longer in a tab; `spills_to_retire` is the decision and is tested. A pipe's spill still lives for the window's life, which is right — a stream the user cannot restart is not the same as a source they can reopen with two keystrokes. |
...
| **The application picker is built — 2026-09-08** | Opening a remote source asks Loki for the values of the `app` label and offers them as a checkbox list, with *Interleave* (one document, `app=~"a\|b"`) and *Separate windows* (one document each) — the owner's ask of 2026-08-31, both halves. `apps::with_apps` rewrites the source's own selector rather than replacing it, so the environment and any pipeline survive; `apps::apps_in` reads the choice back, so reopening shows what was picked. A source that cannot answer the label call still opens whole, with the reason said. |
| **The picker's remaining limit** | Separate windows is capped at `apps::MAX_SEPARATE` (8) because each is a tail with its own poll and its own bounded spill; the refusal says so and names Interleave as the remedy. |
| **Loki asks on a worker — 2026-09-09** | Opening a remote source used to run every request on the message loop: a credential read, a token exchange, the label call, a second token exchange and the query. Measured against the real source, the records fetch alone froze the window for **16.6 seconds**. Both halves are workers now, handed back through `Shell::fetching` and drained on the device-poll tick beside the file reads, with the status bar saying what is being asked. Proven by probing the window with `SendMessageTimeout` throughout: **118 probes answered, none blocked**, where before every one of them would have timed out. |
...
| **Split view** — `Ctrl+\` / `F6`; a tab is one or two panes, stacked or side by side per the edge dropped on, each a whole `Document` on the same path (a second handle + index — said in `CLEANROOM.md`); `Renderer::paint_panes` lays each out and shifts it into place; mouse routed by pane, keyboard to the focused pane; stacked, the top pane has the strip and the bottom the status bar; side by side, both have both and the left owns the status text | `Tab`, `Tabs::split/focus_pane`, `pane_boxes`, `Painter::mark/shift` | headless (`TAILHAWK_SHOT_SPLIT=ERR`) + test |
