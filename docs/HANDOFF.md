...
| **"Follow this trace" is agreed and not built** | `trace_id` and `span_id` are already on every tailed line (`lokiwire.rs` merges Loki's structured metadata into a record's fields), and forty live services emit them. What is missing is the gesture: take the `trace_id` of the line under the caret and filter the view to it. `LOKI.md` §8 carries the measurement and the owner's decision that the *tree* — indentation, subtree selection — is not what a tail app is for. |
